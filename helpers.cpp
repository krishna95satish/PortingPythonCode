...
vector< vector <float> > normalize(vector< vector <float> > grid) {
	...
	vector< vector<float> > newGrid(grid.size(),vector <float> (grid[0].size(),0.0)); // for vector ini
 	double Grid_Sum = 0;

	// todo - your code here
  	for(int grid_rows=0 ; grid_rows <grid.size();grid_rows++)
 		 {
   	 		for(int grid_colms=0 ; grid_colms <grid[0].size();grid_colms++)
    			{
    				Grid_Sum = Grid_Sum + grid[grid_rows][grid_colms];
    			}
  		}
  for(int grid_rows=0 ; grid_rows <grid.size();grid_rows++)
  {
    for(int grid_colms=0 ; grid_colms <grid[0].size();grid_colms++)
    {
    newGrid[grid_rows][grid_colms] = grid[grid_rows][grid_colms]/Grid_Sum;
    }
  }
	return newGrid;
}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	int Temp_x,Temp_y;
	vector < vector <float> > windows {{corner_prob,  adjacent_prob,  corner_prob},
  ...
  for (int i = 0; i<height ; i++)
  {
    ...
    {
        grid_val = grid[i][j];
            for (int x_idx= -1 ;x_idx<2;x_idx++)
            {
              for (int y_idx= -1 ;y_idx<2;y_idx++)
              {
                    mult = windows[x_idx+1][y_idx+1];
                	if(((i + x_idx) % height) < 0)
                    {
                      Temp_x = ((i + x_idx) % height) + height ; // to deal with % in Cpp
                    }
                	else
                    {
                    Temp_x = (i + x_idx) % height;
                    }
            		if(((j + y_idx) % width) < 0)
                    {
                      Temp_y = ((j + y_idx) % width) + width ; // to deal with % in Cpp
                    }
                	else
                    {
                    Temp_y = (j + y_idx) % width;
                    }
                    
                    newGrid[Temp_x][Temp_y] += mult * grid_val;
                }
            }
    }
  }
